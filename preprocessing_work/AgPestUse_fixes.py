...
LowEst_df_filtered.drop('Units', axis=1, inplace=True)
highEst_df_filtered.drop('Units', axis=1, inplace=True)

# State_FIPS_code is not needed since we have the State column
LowEst_df_filtered.drop('State_FIPS_code', axis=1, inplace=True)
highEst_df_filtered.drop('State_FIPS_code', axis=1, inplace=True)

# Group by state and sort by year?
# -----
# -----
